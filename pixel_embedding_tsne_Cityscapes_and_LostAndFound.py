...
for id_cityscapes in range(10):
	...
	chosen_idx = random.sample([x for x in range(features.shape[0])], int(features.shape[0]*0.25))
...
for id_lostAndFound in [4, 27, 45, 50, 51, 61]:
	...
	#plt.show()
	fig.tight_layout()
	fig.savefig('{}/lostAndFound_{}_cityscapes_all_{}.jpg'.format(saved_folder, id_lostAndFound, color))
	plt.close()
